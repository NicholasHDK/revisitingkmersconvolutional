...
class PairDataset(Dataset):
    def __init__(self, file_path, k, neg_sample_per_pos=1000, max_seq_num=0, verbose=True, seed=0):
        ...
        for line in lines:
            ...
            left_kmer_profiles.append(self.__transform_func(left_seq))
            right_kmer_profiles.append(self.__transform_func(right_seq))

        # Nich: pad dense encodings to ensure they're all the same shape, and stack to create self.__kmers
        ...
        self.__kmers = torch.vstack([left_kmer_profiles, right_kmer_profiles]).float()
    ...
    def __getitem__(self, idx):
        ...
        return torch.index_select(self.__kmers, dim=0, index=self.__indices[0, idx]), torch.index_select(self.__kmers, dim=0, index=self.__indices[1, idx]), self.__labels[idx]
...
class VIBModel(torch.nn.Module):
    ...
    def forward(self, left_kmers: torch.Tensor, right_kmers:torch.Tensor):
        ...
        """
        # Nich: extract features
        left_features = self.conv_feature_extractor(left_kmers)
        right_features = self.conv_feature_extractor(right_kmers)
...
def collate_fn(batch):
    ...
    """
    left_reads, right_reads, labels = torch.vstack([item[0] for item in batch]), torch.vstack([item[1] for item in batch]), torch.vstack([item[2] for item in batch])
    left_onehots = batched_dense_to_onehot(left_reads).unsqueeze(1) # add channel dimension
    right_onehots = batched_dense_to_onehot(right_reads).unsqueeze(1)
